...
public class WGraph {
	...
	//Set of vertices
	private final Lista<Vertex> vertices;
	//Set of edges
	private final Lista<Edge> edges;
	...
	@SuppressWarnings("unchecked") private void initializeLists (int size) {
		adjacencies = (Lista<Edge>[]) new Lista[size];
	}
	...
	/**
	 * Given a vertex v, returns its list of adjacencies.
	 * in graph.
	 * @param v The vertex.
	 ...
	 * shortest path tree of the graph.
	 * @return The set of edges in the shortest path tree.
	 */
	public Lista<Edge> shortestPathTree () {
		...
		for (Vertex v : vertices) {
			...
			mste.agrega(new Edge(p,v,v.dist-p.dist));
		}
		return mste;
	}
