...
Para la implementación del proyecto se implementaron las clases:
	...
	* Prim: Contiene el algoritmo de Prim.
...
interfaces ocupadas fueron:
	...
	* MonticuloMinimo: Min Heaps binarios.
	* ComparableIndexable: Interfaz que deben implementar los tipos que se
	  quieran ordenar con los heaps binarios.

La clase Main del paquete Prim es el punto de entrada del programa, en él se
crea la gráfica que se pasa como entrada.
...
$ javac prim/Main.java

Para ejecutar el programa, ejecute el siguiente comando:

$ java prim.Main graph.txt

donde "graph.txt" es el nombre del archivo con la información de la gráfica
sobre la cual desea correr el algoritmo de Prim (se adjunta también un ejemplo,
no es necesario usar el mismo nombre de archivo, ni que esté en el mismo
directorio, pero en tal caso debe dar la ruta relativa/total del archivo).
...
donde <u> y <v> son las etiquetas de los vértices iniciales, es decir, es la
arista (u,v) (una arista dirigida, el programa se encarga de crear la arista
(v,u) con el mismo peso para representar aristas no dirigidas como lo requiere
el algoritmo), y <<w>> es el peso de la arista, el cual es opcional (si no se
proporciona un peso la arista será creada con un peso de 1 por defecto, el peso
w debe ser un entero no negativo).
...
La salida del programa son las aristas del árbol con su respectivo peso en
formato:
